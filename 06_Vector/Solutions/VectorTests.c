...
void _TEST_CountElements()
{
	...
	vector->data[3] = 561329;
	vector->size = 4;
	assert(CountElements(vector, 10) == 2);
	assert(CountElements(vector, 5) == 0);
	printf("%s passed successfully.\n\n", __func__);
}

void _TEST_ContainsElement()
{
	printf("%s started...\n", __func__);
	Vector_int * vector = CreateVector(4);
	vector->data[0] = 2;
	...
	vector->data[3] = 561329;
	vector->size = 4;
	assert(ContainsElement(vector, 2));
	assert(ContainsElement(vector, -3));
	assert(ContainsElement(vector, 10));
	assert(ContainsElement(vector, 561329));
	assert(!ContainsElement(vector, 5));
	DeleteVector(vector);
	printf("%s passed successfully.\n\n", __func__);
}
